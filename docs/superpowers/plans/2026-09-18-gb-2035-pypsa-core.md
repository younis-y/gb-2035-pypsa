...
def _from_technology_data(td: pd.DataFrame, name: str, eur_to_gbp: float) -> dict[str, object]:
    ...
    return {
        ...
        "vom_gbp_per_mwh": float(sub.loc["VOM", "value"]) * eur_to_gbp
        if "VOM" in sub.index
        else 0.0,
...
def capital_cost_per_yr(costs: pd.DataFrame, technology: str, rate: float) -> float:
    ...
    return annuity(rate, float(row["lifetime_yr"])) * float(row["capex_gbp"]) + float(
        row["fom_gbp_per_yr"]
    )
...
def battery_capital_cost_per_mw_yr(costs: pd.DataFrame, hours: float, rate: float) -> float:
    ...
    per_mwh = annuity(rate, float(storage["lifetime_yr"])) * float(storage["capex_gbp"]) + float(
        storage["fom_gbp_per_yr"]
    )
...
def add_links(
    ...
    if scenario.transmission_expandable:
        ...
        for name, row in links.iterrows():
            ...
            per_mw_yr = (
                annuity(settings.discount_rate, float(c["lifetime_yr"])) * float(c["capex_gbp"])
                + float(c["fom_gbp_per_yr"])
            ) * km
...
def add_thermal(
    ...
    for z in LAND_ZONES:
        ...
        for tech, existing_key, eff in (
            ...
            ("ocgt", "ocgt", float(c.at["ocgt", "efficiency"])),
        ):
            ...
            if mw > 0:
                n.add(
                    ...
                    p_nom_max=mw,
                    efficiency=eff,
                    capital_cost=float(c.at[existing_key, "fom_gbp_per_yr"]),
                    marginal_cost=gas / eff
                    + float(c.at[existing_key, "vom_gbp_per_mwh"])
        ...
        n.add(
            ...
            + float(c.at["ocgt", "vom_gbp_per_mwh"])
        ...
        n.add(
            ...
            + float(c.at["gas_ccs", "vom_gbp_per_mwh"])
...
def add_hydrogen_node(
    ...
    n.add(
        ...
        bus,
        carrier="H2",
        x=float(inputs.buses.at[zone, "x"]) + H2_BUS_OFFSET,
        y=float(inputs.buses.at[zone, "y"]),
    )
    n.add(
        "Link",
        f"electrolysis {name}",
        ...
        rows.append(
            {
                ...
                "name": name,
                "carrier": l["carrier"],
                "zone": f"{l['bus0']}->{l['bus1']}",
                "p_nom_opt": float(l["p_nom_opt"]),
                "p_nom_min": float(l["p_nom_min"]),
                "unit": "MW",
            }
        )
            ...
            rows.append(
                {
                    ...
                    "value": float((n.loads_t.p_set[demand] * w).sum()) / MWH_PER_TWH,
                }
            )
...
def dispatch_hourly(n: pypsa.Network) -> pd.DataFrame:
    ...
    out["load"] = n.loads_t.p_set[
        [
            c
            for c in n.loads.index
            if n.loads.at[c, "bus"] in n.buses[n.buses["carrier"] == "AC"].index
        ]
    ].sum(axis=1)
    out["price_mean_gbp_per_mwh"] = n.buses_t.marginal_price[
...
def summary_row(n: pypsa.Network, scenario_name: str, solve: SolveResult) -> pd.DataFrame:
    ...
    row = {
        ...
        "curtailed_share_wind_solar": float(
            curtailment(n).eval("(available_twh - used_twh)").sum()
...
def build_derived(root: RootOpt = Path()) -> None:
    ...
    ).to_csv(derived / "fleet_by_zone.csv", index=False)
    build_repd_by_zone(raw / "REPD_Publication_Q2_2026.csv", zones).to_csv(
        derived / "repd_by_zone.csv", index=False
    )
    ...
    extract_fes_2035(raw / "fes2025_data_workbook.xlsx").to_csv(
        derived / "fes2025_gb_2035.csv", index=False
    )
    build_costs(
        raw / "costs_2035.csv", paths.config / "costs_overrides.csv", settings.eur_to_gbp
    ).to_csv(derived / "costs_2035_gb.csv")
...
def build_network_cmd(
    ...
    console.print(
        f"{len(n.buses)} buses, {len(n.generators)} generators, {len(n.snapshots)} snapshots -> {out / 'network_unsolved.nc'}"
    )
...
def _run(paths: ProjectPaths, settings: Settings, sc: Scenario, results: Path) -> None:
    ...
    console.print(
        f"[green]{sc.name}[/green]: optimal in {seconds:.0f}s, total cost {result.total_cost_gbp_per_yr / 1e9:.2f} bn GBP/yr -> {out}"
    )
...
def extract_cmd(
    ...
    constant = float(getattr(n, "objective_constant", 0.0) or 0.0)
    write_results(
        extract_all(n, sc.name, SolveResult("ok", "optimal", float(n.objective), constant)), out
    )
...
TECH_COLS = [
    ("offwind_gw", "Offshore wind"),
    ("onwind_gw", "Onshore wind"),
    ("solar_gw", "Solar"),
    ("nuclear_gw", "Nuclear"),
    ("gas_ccs_gw", "Gas CCS"),
    ("gas_gw", "Unabated gas"),
    ("battery_gw", "Battery"),
    ("h2_turbine_gw_el", "H2 turbine"),
]
...
def optimised_vs_fes(
    ...
    pairs = [
        (
            "Offshore wind",
            row["offwind_gw"],
            fes_value(fes, "offshore_wind_gw", "Holistic Transition"),
        ),
        (
            "Onshore wind",
            row["onwind_gw"],
            fes_value(fes, "onshore_wind_gw", "Holistic Transition"),
        ),
    ...
    ax.bar(
        [i + 0.2 for i in x], [p[2] for p in pairs], width=0.4, label="FES 2025 Holistic Transition"
    )
...
def storage_by_cap(summary: pd.DataFrame, out: Path) -> Path:
    ...
    ax.plot(
        df["scenario"].astype(str), df["h2_turbine_gw_el"], marker="s", label="H2 turbine (GW el)"
    )
    ax.plot(
